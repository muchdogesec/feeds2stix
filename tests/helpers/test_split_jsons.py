...
@pytest.mark.parametrize("max_size_kb,expected", [
...
def test_save_bundle_writes_json_file(tmp_path):
    path = split_jsons.save_bundle(str(tmp_path), "bundle--1", "name", 1, [{"type": "indicator", "id": "indicator--1"}])
...
def test_split_stix_bundle_creates_multiple_files(tmp_path):
    objects = [
        ...
        {"type": "marking-definition", "id": "marking-definition--1", "definition_type": "statement", "definition": {"statement": "x"}},
    ]
    for i in range(8):
        objects.append(
            {
    ...
    out = split_jsons.split_stix_bundle(str(input_file), max_size_kb=11, output_dir=str(tmp_path / "out"))
    assert len(out) > 1
...
def test_split_stix_bundle_tiny_max_size_does_not_stall(tmp_path):
    ...
    out = split_jsons.split_stix_bundle(str(input_file), max_size_kb=1, output_dir=str(tmp_path / "out_tiny"))
    assert len(out) >= 1
    assert all(Path(p).exists() for p in out)
