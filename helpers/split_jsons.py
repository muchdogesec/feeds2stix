...
def split_stix_bundle(input_file, max_size_kb, output_dir=None):
    ...
    logger.info(f"Metadata objects: {len(metadata_objects)}, Data objects: {len(data_objects)}")
    ...
    if base_size_kb > max_size_kb:
        logger.warning(f"Base bundle size ({base_size_kb:.2f} KB) exceeds max size ({max_size_kb} KB)")
        logger.warning("Will attempt to split anyway, but some bundles may exceed max size")
    ...
    while i < len(data_objects):
        ...
        if not batch_added and current_chunk:
            output_file = save_bundle(output_dir, bundle.get('id', 'bundle--00000000-0000-0000-0000-000000000000'), base_name, chunk_num, metadata_objects + current_chunk)
...
def save_bundle(output_dir, bundle_id, base_name, chunk_num, objects):
    ...
    logger.info(f"Created {output_file} with {len(objects)} objects ({actual_size:.2f} KB)")
    return output_file
