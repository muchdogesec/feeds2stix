...
def create_stix_objects(
    ...
    for idx, domain in enumerate(domains):
        ...
        indicator_name = f"Domain: {domain}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ThreatView Domain threat intelligence feed to STIX 2.1 format"
    )
    ...
    try:
        ...
        bundle_path = save_bundle_to_file(bundle, output_dir, "threatview_domain")
        ...
        logger.error(f"Error processing ThreatView Domain feed: {e}", exc_info=True)
        return 1
